...
class Board(object):
    def __init__(self, position=[350, 500], theta=0, l_board=80, w_board=24, max_speed_mi_ph=20, wheel_diameter_cm=5.5, zero_to_max_vel_time_dt=10, dt=.01, wheel_tuples=[]):
        ...
        self.wheel_diameter_cm = wheel_diameter_cm # cm
        self.zero_to_max_vel_time_dt = zero_to_max_vel_time_dt # s
        ...
        self.ang_accel_rad_ps2 = self.max_angular_velocity_rad_ps / self.zero_to_max_vel_time_dt # rad / s^2
...
class TwoWheelBoard(Board):
    def __init__(self, position=[350, 500], theta=0, l_board=80, w_board=24, max_speed_mi_ph=20, wheel_diameter_cm=5.5, zero_to_max_vel_time_dt=4, dt=.01, wheel_tuples=[]):
        ...
        super().__init__(position, theta, l_board, w_board, max_speed_mi_ph, wheel_diameter_cm, zero_to_max_vel_time_dt, dt, wheel_tuples)
    ...
    def move_wheels_towards_targets(self):
        ...
        if move_wheel_vels_together or move_wheel_vels_close:
            print(f"move_wheel_vels_together or move_wheel_vels_close")
        ...
        if move_wheel_vels_apart:
            ...
            if self.target_diff_ang_vel >= 0:
                L_wheel.accelerate_towards_limit(vel_limit=self.target_ang_vel + self.target_diff_ang_vel/2)
                R_wheel.accelerate_towards_limit(vel_limit=L_wheel.curr_ang_vel - self.target_diff_ang_vel)
            else:
                R_wheel.accelerate_towards_limit(vel_limit=self.target_ang_vel - self.target_diff_ang_vel/2)
                L_wheel.accelerate_towards_limit(vel_limit=R_wheel.curr_ang_vel + self.target_diff_ang_vel)
                # L_wheel.accelerate_towards_limit(vel_limit=-self.target_ang_vel + self.target_diff_ang_vel/2)
                # R_wheel.accelerate_towards_limit(vel_limit=L_wheel.curr_ang_vel - self.target_diff_ang_vel)
        ...
        print(f"Wheel vel avg: {(L_wheel.curr_ang_vel + R_wheel.curr_ang_vel) / 2}")
        print(f"self.target_ang_vel: {self.target_ang_vel}")
